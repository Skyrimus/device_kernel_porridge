...
/*----------------------------------------------------------------------------*/
static const struct file_operations kx023_fops = {
	.owner = THIS_MODULE,
	.open = kx023_open,
...
static int kx023_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	int err = 0;
	GSE_FUN();    
	printk("FIXED FIXED FIXED FIXED FIXED");
	printk("Skyrimus was here");
	...
	{   
		...
		err = KX023_SetPowerMode(obj->client, false);
		if (err)
		{
...
static int kx023_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct kx023_i2c_data *obj = NULL;
